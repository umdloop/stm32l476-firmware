...
void heartbeat_system_controller(void)
{
     ...
     */
    int32_t hb_s = 0;
    if (!CanParams_GetInt32("SPECTROSCOPY_PCB_C.pcb_heartbeat_delay", &hb_s))
        return;
    ...
    if (delay_elapsed(&s_heartbeat_timer, hb_ms))
    {
         ...
         */
        (void)CanParams_SetBool("SPECTROSCOPY_PCB_R.pcb_heartbeat_success", true);
        (void)CanSystem_Send("SPECTROSCOPY_PCB_R.pcb_heartbeat_success");
    }
}
